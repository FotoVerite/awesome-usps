...
describe AwesomeUsps do
  ...
  it "Find all domestic rates for two packages", :vcr do
    USPS.canned_domestic_rates_test.size.should == 2
  end

  it "Find all international rates for two packages", :vcr do
    USPS.canned_world_rates_test.size.should == 2
  end

  it "Verify that an address is accurate", :vcr do
  ...
  it "Finds an address by zipcode", :vcr do
  ...
  it "should return a signature label", :vcr do
    result = USPS.canned_signature_confirmation_label_test
    result[:image_type].should == "application/pdf"
    result[:label].should_not be_nil
  end

  it "should return a merch label", :vcr do
